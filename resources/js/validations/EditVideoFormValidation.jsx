/**
 * 動画コンテンツ編集フォームバリデーション
 * 
 * @param {Object} data フォームの入力情報
 * @return {Array} バリデーションメッセージ
 */
const EditVideoFormValidation = (data) => {
    ...
    if(data.thumbnail.file !== "" && data.thumbnail.path !== "") {
        if (data.thumbnail.file != "" && !data.thumbnail.file.type.startsWith('image/')) {
    ...
    if(data.video.file === "" && data.video.path === "") {
        ...
        if (data.video.file != "" && !data.video.file.type.startsWith('video/')) {
...
export default EditVideoFormValidation;
